...
#-- First coding the logic for only one image
...
'''
Using the basic CNN architecture, the following performance metrics are 
observed:
    a. Accuracy of model on training set (best after 50 epochs): 100%   <<--
    b. Accuracy of model on validation set (best after 50 epochs): 82%
    c. Accuracy of the model on test set: 85%

----------------------------------------------------------------------------
Comments (for later review): 
    
Just by looking at the training and validation set performance, the model is 
definitely overfitting.

-->> Need to explore different techniques that would prevent the model from 
overfitting.
